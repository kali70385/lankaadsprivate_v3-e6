...
export default function AdComponent({ ad }: { ad: Ad }) {
  ...
  return (
    <Link href={`/ad/${ad.id}`} className="block w-full">
      <div className="w-full h-36 sm:h-40 md:h-44 border border-rose-100 rounded-lg bg-white/90 backdrop-blur-sm shadow-sm hover:shadow-md transition-shadow overflow-hidden">
        {/* Fixed rectangular layout - horizontal always */}
        <div className="flex h-full w-full">
          {/* Left side - Image (fixed width) */}
          <div className="w-32 sm:w-36 md:w-40 h-full relative flex-shrink-0">
            <Image
          ...
          {/* Right side - Details (flexible width with proper spacing) */}
          <div className="flex-1 p-3 sm:p-4 min-w-0 flex flex-col justify-between overflow-hidden">
            ...
            <div className="flex-1 min-h-0 space-y-1 sm:space-y-2">
              ...
              <div className="flex items-center flex-shrink-0">
                <Clock className="w-3 h-3 mr-1 text-gray-500 flex-shrink-0" />
                <span className="text-gray-500 text-xs truncate">{timeAgo}</span>
              </div>

              {/* Title - allow 2 lines max */}
              <h3 className="text-sm sm:text-base font-semibold text-primary line-clamp-2 break-words leading-tight">
                {ad.title}
              </h3>

              {/* Description - flexible height but limited */}
              <p className="text-xs sm:text-sm text-primary/90 line-clamp-2 sm:line-clamp-3 break-words leading-relaxed flex-1 min-h-0">
                {ad.description}
              </p>
            </div>

            {/* Bottom section - fixed height, always visible */}
            <div className="flex-shrink-0 space-y-1 mt-2 pt-2 border-t border-gray-100">
              {/* Price - prominent display */}
              <div className="flex items-center justify-between">
                <p className="text-sm sm:text-base md:text-lg font-bold text-primary/80 truncate">
              ...
              <div className="flex items-center justify-between gap-2 text-xs">
                <div className="flex items-center min-w-0 flex-1">
                  <span className="inline-block w-2 h-2 rounded-full bg-primary/60 mr-2 flex-shrink-0"></span>
                ...
                {/* Location */}
                {ad.district && (
                  <div className="flex items-center min-w-0 flex-shrink-0 max-w-24 sm:max-w-32">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="w-3 h-3 text-primary/90 mr-1 flex-shrink-0"
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="2"
                    ...
                    <span className="text-primary/90 text-xs truncate">
              ...
              <div className="flex items-center justify-between gap-2 text-xs">
                <div className="flex items-center min-w-0 flex-1">
                  <PhoneIcon className="w-3 h-3 text-primary/90 mr-1 flex-shrink-0" />
                  <span className="text-primary/90 truncate">{ad.contactNumber}</span>
                </div>
